...
import com.qwj.girl.dao.redis.BadTeacherDao;
...
@Component
public class TeacherUncensoredActresses implements PageProcessor {

    @Resource
    public BadTeacherDao badTeacherDao;
    ...
    public Map<String, BadTeacher> map = new HashMap<>();
    ...
    public void process(Page page) {
        ...
        if (url.contains("uncensored/actresses")) {
            ...
            for (Element div : divs) {
                BadTeacher badTeacher = new BadTeacher();
                Element a = div.selectFirst("a");
                String href = a.attr("href").trim();
                String code = href.substring(href.lastIndexOf("/") + 1).trim();
                List<BadTeacher> list = badTeacherDao.findByCode(code);
                int count = list.size();
                if (count > 0) {
                    continue;
                }
                badTeacher.setCode(code);
                badTeacher.setDetailLink(href);
                map.put(href, badTeacher);
