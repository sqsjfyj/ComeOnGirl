...
import com.qwj.girl.dao.mongo.BadActorDao;
...
@Component
public class GirlUncensoredActresses implements PageProcessor {

    @Resource
    public BadActorDao badActorDao;
    ...
    public Map<String, BadActor> map = new HashMap<>();
    ...
    public void process(Page page) {
        ...
        if (url.contains("uncensored/actresses")) {
            ...
            for (Element div : divs) {
                BadActor badActor = new BadActor();
                Element a = div.selectFirst("a");
                String href = a.attr("href").trim();
                String code = href.substring(href.lastIndexOf("/") + 1).trim();
                int count = badActorDao.countByCode(code);
                if (count > 0) {
                    continue;
                }
                badActor.setCode(code);
                badActor.setDetailLink(href);
                map.put(href, badActor);
